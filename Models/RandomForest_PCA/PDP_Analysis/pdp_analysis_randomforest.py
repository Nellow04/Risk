...
class PDPAnalysis:
    ...
    def analyze_nonlinear_effects(self, top_components, pd_results_list):
        ...
        for i, comp_name in enumerate(top_components[:8]):  # Analizza top 8
            ...
            print(f"{comp_name}: Range={effect_range:.4f}, Non-linearità={nonlinearity_score:.4f}, Soglie={len(critical_points)}")

        return nonlinear_analysis

    def _interpret_nonlinear_effect(self, effect_range, nonlinearity_score, critical_points):
        """Interpreta l'effetto non-lineare"""
        interpretations = []

        if effect_range > 0.1:
            interpretations.append("Forte impatto sul rischio")
        elif effect_range > 0.05:
            interpretations.append("Impatto moderato sul rischio")
        else:
            interpretations.append("Impatto limitato sul rischio")

        if nonlinearity_score > 0.001:
            interpretations.append("Relazione altamente non-lineare")
        elif nonlinearity_score > 0.0001:
            interpretations.append("Relazione moderatamente non-lineare")
        else:
            interpretations.append("Relazione prevalentemente lineare")

        if len(critical_points) > 3:
            interpretations.append("Multiple soglie critiche identificate")
        elif len(critical_points) > 0:
            interpretations.append("Soglie critiche presenti")
    ...
    def save_results(self, importance_df, nonlinear_analysis):
        ...
        with open('./pdp_analysis_report.txt', 'w') as f:
            f.write("PARTIAL DEPENDENCE PLOTS ANALYSIS - RANDOM FOREST PCA\n")
            f.write("="*55 + "\n\n")

            f.write("TOP 10 COMPONENTI PIÙ IMPORTANTI:\n")
            f.write("-"*35 + "\n")
            for i, row in importance_df.head(10).iterrows():
                comp_desc = self.get_component_description(row['Component'])
                f.write(f"{i+1:2d}. {row['Component']:4s}: {row['Permutation_Importance']:.4f} - {comp_desc}\n")

            f.write("\n\nANALISI EFFETTI NON-LINEARI:\n")
            f.write("-"*30 + "\n")
            for comp, data in nonlinear_analysis.items():
                f.write(f"\n{comp} - {data['component_description']}:\n")
                f.write(f"  Effect Range: {data['effect_range']:.4f}\n")
                f.write(f"  Nonlinearity Score: {data['nonlinearity_score']:.6f}\n")
                f.write(f"  Critical Points: {data['n_critical_points']}\n")
                f.write(f"  Interpretazione: {data['interpretazione']}\n")

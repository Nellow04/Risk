    ...
    for i, idx in enumerate(sample_indices):
        try:
            ...
            for feature_desc, importance in explanation_list:
                # Estrai numero della componente (es. "PC8 <= 0.17" -> 7 per PC8)
                try:
...
def create_lime_visualizations(feature_importances, feature_names, explanations):
    ...
    bars = plt.bar(range(len(top_10_names)), top_10_importance, alpha=0.8, color='blue')
    plt.xlabel('Componenti PCA')
    plt.ylabel('Importanza Media LIME')
    plt.title('Top 10 Feature Importance - LIME Ensemble PCA')
    ...
    plt.title('Heatmap Importanza Features - LIME Ensemble PCA')
    plt.xlabel('Campioni')
    ...
    fig.suptitle('LIME Analysis Overview - Ensemble PCA', fontsize=16, fontweight='bold')

...
def create_anchors_explainer(X_train, feature_names, model):
    try:
        # Crea l'explainer semplificato per ensemble PCA
...
def analyze_anchors_rules(explanations):
    ...
    for i, (rule, count) in enumerate(top_rules[:5], 1):
        print(f"   {i}. {rule} (freq: {count})")
...
def create_anchors_visualizations(explanations, stats):
    ...
    fig.suptitle('🎯 Anchors Analysis - Ensemble PCA Model', fontsize=16, fontweight='bold')

    # 1. Distribuzione Precisione vs Copertura
    precisions = [exp['precision'] for exp in explanations]
    coverages = [exp['coverage'] for exp in explanations]

    axes[0, 0].scatter(coverages, precisions, alpha=0.6, s=50, color='blue')
    ...
    if stats['top_rules']:
        ...
        axes[1, 0].barh(rule_names, rule_counts, color='green', alpha=0.7)
...
def main():
    ...
    print ("Anaisi Anchors completata")
